...
static Value fn_matdisp(std::vector<Value>& args, Interpreter&) {
    for (auto& a : args) {
...
static Value fn_matadd(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2) throw Error{"scientific", -1, "matadd: at least 2 matrices required"};
    Matrix<Real> a = arr2matrix(args[0], "matadd");
    for (std::size_t i = 1; i < args.size(); ++i) {
        Matrix<Real> b = arr2matrix(args[i], "matadd");
        if (a.rows() != b.rows() || a.cols() != b.cols())
            throw Error{"scientific", -1, "matadd: shape mismatch"};
        a = a + b;
    }
    return matrix2arr(a);
}

static Value fn_matsub(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2) throw Error{"scientific", -1, "matsub: at least 2 matrices required"};
    Matrix<Real> a = arr2matrix(args[0], "matsub");
    for (std::size_t i = 1; i < args.size(); ++i) {
        Matrix<Real> b = arr2matrix(args[i], "matsub");
        if (a.rows() != b.rows() || a.cols() != b.cols())
            throw Error{"scientific", -1, "matsub: shape mismatch"};
        a = a - b;
    }
    return matrix2arr(a);
}

static Value fn_matmul(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2) throw Error{"scientific", -1, "matmul: at least 2 matrices required"};
    Matrix<Real> a = arr2matrix(args[0], "matmul");
    for (std::size_t i = 1; i < args.size(); ++i) {
        Matrix<Real> b = arr2matrix(args[i], "matmul");
        if (a.cols() != b.rows())
            throw Error{"scientific", -1, "matmul: nonconformant dimensions"};
        a = a * b;
    }
    return matrix2arr(a);
}

static Value fn_hadamard(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2) throw Error{"scientific", -1, "hadamard: at least 2 matrices required"};
    Matrix<Real> a = arr2matrix(args[0], "hadamard");
    for (std::size_t i = 1; i < args.size(); ++i) {
        Matrix<Real> b = arr2matrix(args[i], "hadamard");
        if (a.rows() != b.rows() || a.cols() != b.cols())
            throw Error{"scientific", -1, "hadamard: shape mismatch"};
...
static Value fn_transpose(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "transpose: 1 argument required"};
    return matrix2arr(arr2matrix(args[0], "transpose").transpose());
}

static Value fn_nrows(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "nrows: 1 argument required"};
    return NumVal{(double)arr2matrix(args[0], "nrows").rows()};
}

static Value fn_ncols(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "ncols: 1 argument required"};
    return NumVal{(double)arr2matrix(args[0], "ncols").cols()};
}

static Value fn_matsum(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "matsum: 2 arguments required (matrix, axis)"};
    Matrix<Real> a = arr2matrix(args[0], "matsum");
    int axis = (int)scalar(args[1], "matsum");
    if (axis != 0 && axis != 1) throw Error{"scientific", -1, "matsum: axis must be 0 or 1"};
    return matrix2arr(a.sum(axis));
}

static Value fn_getrows(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 3) throw Error{"scientific", -1, "getrows: 3 arguments required"};
    ...
    if (start < 0 || start >= (int)a.rows() || end < start || end >= (int)a.rows())
        throw Error{"scientific", -1, "getrows: invalid row range"};
    return matrix2arr(a.get_rows(start, end));
}

static Value fn_getcols(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 3) throw Error{"scientific", -1, "getcols: 3 arguments required"};
    ...
    if (start < 0 || start >= (int)a.cols() || end < start || end >= (int)a.cols())
        throw Error{"scientific", -1, "getcols: invalid column range"};
    return matrix2arr(a.get_cols(start, end));
}

// ── Matrix construction ───────────────────────────────────────────────────────

static Value fn_eye(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "eye: 1 argument required"};
    int n = (int)scalar(args[0], "eye");
    if (n <= 0) throw Error{"scientific", -1, "eye: size must be positive"};
...
static Value fn_rand(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 1 || args.size() > 2)
        throw Error{"scientific", -1, "rand: 1 or 2 arguments required"};
    int len  = (int)scalar(args[0], "rand");
    int rows = args.size() == 2 ? (int)scalar(args[1], "rand") : 1;
    if (len <= 0 || rows <= 0) throw Error{"scientific", -1, "rand: dimensions must be positive"};
    if (rows == 1) {
        NumVal out(len);
        for (int i = 0; i < len; ++i)
...
static Value fn_zeros(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 1 || args.size() > 2)
        throw Error{"scientific", -1, "zeros: 1 or 2 arguments"};
    int len  = (int)scalar(args[0], "zeros");
    int rows = args.size() == 2 ? (int)scalar(args[1], "zeros") : 1;
    if (len <= 0 || rows <= 0) throw Error{"scientific", -1, "zeros: dimensions must be positive"};
...
static Value fn_ones(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 1 || args.size() > 2)
        throw Error{"scientific", -1, "ones: 1 or 2 arguments"};
    int len  = (int)scalar(args[0], "ones");
    int rows = args.size() == 2 ? (int)scalar(args[1], "ones") : 1;
    if (len <= 0 || rows <= 0) throw Error{"scientific", -1, "ones: dimensions must be positive"};
...
static Value fn_bpf(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 3) throw Error{"scientific", -1, "bpf: at least 3 arguments: init, len, end"};
    Real init = scalar(args[0], "bpf");
    int  len0 = (int)scalar(args[1], "bpf");
    Real end0 = scalar(args[2], "bpf");
    if (len0 <= 0) throw Error{"scientific", -1, "bpf: segment length must be positive"};
    std::size_t remaining = args.size() - 3;
    if (remaining % 2 != 0) throw Error{"scientific", -1, "bpf: extra args must be (len, end) pairs"};
    BPF<Real> bpf(len0);
    ...
    for (std::size_t i = 0; i < remaining / 2; ++i) {
        ...
        if (seg_len <= 0) throw Error{"scientific", -1, "bpf: segment length must be positive"};
...
static Value fn_inv(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "inv: 1 argument required"};
    Matrix<Real> a = arr2matrix(args[0], "inv");
    if (a.rows() != a.cols()) throw Error{"scientific", -1, "inv: matrix must be square"};
    return matrix2arr(a.inverse());
}

static Value fn_det(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "det: 1 argument required"};
    Matrix<Real> a = arr2matrix(args[0], "det");
    if (a.rows() != a.cols()) throw Error{"scientific", -1, "det: matrix must be square"};
    return NumVal{a.det()};
}

// diag(v)  -> diagonal matrix (NumVal -> matrix)
// diag(M)  -> extract diagonal (matrix -> NumVal)
static Value fn_diag(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "diag: 1 argument required"};
    ...
    throw Error{"scientific", -1, "diag: argument must be vector or matrix"};
}

static Value fn_rank(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "rank: 1 argument required"};
...
static Value fn_solve(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "solve: 2 arguments required (matrix, vector)"};
    Matrix<Real> A = arr2matrix(args[0], "solve");
    const NumVal& b = nvec(args[1], "solve");
    const std::size_t n = A.rows();
    if (A.cols() != n) throw Error{"scientific", -1, "solve: A must be square"};
    if (b.size() != n) throw Error{"scientific", -1, "solve: b length must equal number of rows"};
    Matrix<Real> M = A;
    ...
    for (std::size_t k = 0; k < n; ++k) {
        ...
        if (maxv < eps) throw Error{"scientific", -1, "solve: singular matrix"};
...
static Value fn_matcol(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "matcol: 2 arguments required"};
    Matrix<Real> a = arr2matrix(args[0], "matcol");
    int col = (int)scalar(args[1], "matcol");
    if (col < 0 || col >= (int)a.cols()) throw Error{"scientific", -1, "matcol: column index out of range"};
    NumVal out(a.rows());
...
static Value fn_stack2(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "stack2: 2 arguments required"};
    const NumVal& x = nvec(args[0], "stack2");
    const NumVal& y = nvec(args[1], "stack2");
    if (x.size() != y.size()) throw Error{"scientific", -1, "stack2: x and y must have same length"};
    std::size_t n = x.size();
    Matrix<Real> m(n, 2);
...
static Value fn_hstack(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2) throw Error{"scientific", -1, "hstack: at least 2 matrices required"};
    ...
    for (std::size_t i = 1; i < args.size(); ++i) {
        ...
        if (m.rows() != rows) throw Error{"scientific", -1, "hstack: row count mismatch"};
...
static Value fn_vstack(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2) throw Error{"scientific", -1, "vstack: at least 2 matrices required"};
    Matrix<Real> a = arr2matrix(args[0], "vstack");
    std::size_t cols = a.cols(), total_rows = a.rows();
    std::vector<Matrix<Real>> mats{a};
    for (std::size_t i = 1; i < args.size(); ++i) {
        Matrix<Real> m = arr2matrix(args[i], "vstack");
        if (m.cols() != cols) throw Error{"scientific", -1, "vstack: column count mismatch"};
...
static Value fn_median(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "median: 2 arguments required (vector, order)"};
    const NumVal& v = nvec(args[0], "median");
    int order = (int)scalar(args[1], "median");
    if (order <= 0) throw Error{"scientific", -1, "median: order must be positive"};
...
static Value fn_linefit(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "linefit: 2 arguments required"};
    const NumVal& x = nvec(args[0], "linefit");
    const NumVal& y = nvec(args[1], "linefit");
    if (x.size() != y.size()) throw Error{"scientific", -1, "linefit: x and y must have same size"};
    LineFit<Real> line;
    std::valarray<Real> xv(x), yv(y);
    if (!line.fit(xv, yv)) throw Error{"scientific", -1, "linefit: cannot fit (vertical line)"};
...
static Value fn_norm(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 1 || args.size() > 2) throw Error{"scientific", -1, "norm: 1 or 2 arguments"};
    const NumVal& v = nvec(args[0], "norm");
    int p = args.size() == 2 ? (int)scalar(args[1], "norm") : 2;
    if (p <= 0) throw Error{"scientific", -1, "norm: p must be positive"};
...
static Value fn_dist(std::vector<Value>& args, Interpreter&) {
    if (args.size() < 2 || args.size() > 3) throw Error{"scientific", -1, "dist: 2 or 3 arguments"};
    const NumVal& x = nvec(args[0], "dist");
    const NumVal& y = nvec(args[1], "dist");
    if (x.size() != y.size()) throw Error{"scientific", -1, "dist: x and y must have same length"};
    int p = args.size() == 3 ? (int)scalar(args[2], "dist") : 2;
    if (p <= 0) throw Error{"scientific", -1, "dist: p must be positive"};
...
static Value fn_matmean(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "matmean: 2 arguments required"};
    ...
    throw Error{"scientific", -1, "matmean: axis must be 0 or 1"};
}

// matstd(M, axis) -> NumVal: std dev along axis 0 or 1
static Value fn_matstd(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "matstd: 2 arguments required"};
    ...
    throw Error{"scientific", -1, "matstd: axis must be 0 or 1"};
}

// cov(M) -> covariance matrix of columns (rows = observations)
static Value fn_cov(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "cov: 1 argument required"};
    Matrix<Real> a = arr2matrix(args[0], "cov");
    std::size_t n = a.rows(), d = a.cols();
    if (n < 2) throw Error{"scientific", -1, "cov: need at least 2 rows"};
    NumVal mu(d);
...
static Value fn_corr(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "corr: 1 argument required"};
    Matrix<Real> a = arr2matrix(args[0], "corr");
    std::size_t n = a.rows(), d = a.cols();
    if (n < 2) throw Error{"scientific", -1, "corr: need at least 2 rows"};
    NumVal mu(d), sigma(d);
...
static Value fn_zscore(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "zscore: 1 argument required"};
...
static Value fn_pca(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 1) throw Error{"scientific", -1, "pca: 1 argument required"};
    Matrix<Real> X = arr2matrix(args[0], "pca");
    int rows = (int)X.rows(), cols = (int)X.cols();
    if (rows == 0 || cols == 0) throw Error{"scientific", -1, "pca: empty matrix"};
...
static Value fn_kmeans(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 2) throw Error{"scientific", -1, "kmeans: 2 arguments required (matrix, K)"};
    Matrix<Real> X = arr2matrix(args[0], "kmeans");
    int K = (int)scalar(args[1], "kmeans");
    int n = (int)X.rows(), m = (int)X.cols();
    if (n == 0 || m == 0) throw Error{"scientific", -1, "kmeans: empty matrix"};
    if (K <= 0 || K > n) throw Error{"scientific", -1, "kmeans: invalid K"};
...
static Value fn_knn(std::vector<Value>& args, Interpreter&) {
    if (args.size() != 3) throw Error{"scientific", -1, "knn: 3 arguments required (train, K, queries)"};
    if (!std::holds_alternative<ArrayPtr>(args[0]))
        throw Error{"scientific", -1, "knn: training data must be an array"};
    const auto& train_arr = std::get<ArrayPtr>(args[0])->elems;
    int obs = (int)train_arr.size();
    if (obs < 1) throw Error{"scientific", -1, "knn: insufficient training data"};
    int K = (int)scalar(args[1], "knn");
    if (K < 1 || K > obs) throw Error{"scientific", -1, "knn: invalid K"};
    // Determine feature dimension from first training sample
    if (!std::holds_alternative<ArrayPtr>(train_arr[0]))
        throw Error{"scientific", -1, "knn: each training item must be [features, label]"};
    const auto& first = std::get<ArrayPtr>(train_arr[0])->elems;
    if (first.size() != 2 || !std::holds_alternative<NumVal>(first[0]))
        throw Error{"scientific", -1, "knn: training item[0] must be [NumVec, label]"};
    ...
    if (!std::holds_alternative<ArrayPtr>(args[2]))
        throw Error{"scientific", -1, "knn: queries must be an array"};
