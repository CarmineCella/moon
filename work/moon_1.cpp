...
enum TK {
    NUM, STR, IDENT,
    VAR, PROC, WHILE, RETURN, PRINT,  IF, ELSE,
    AND, OR, NOT,    
...
struct Interp {
    ...
    void if_stmt() {
        consume();
        expect(LPAREN); bool taken = std::get<double>(expr ()) != 0.0; expect(RPAREN);
        if (taken) run_block(); else skip_block();
        while (check(ELSE)) {
            consume();
            if (check(IF)) {
                consume();
                expect(LPAREN); bool c = std::get<double>(expr ()); expect(RPAREN);
