...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{Musil v0.51 User Manual}}
...
\end{center}
{\large\color{gray} User Manual \quad v0.51}\par
\vspace{3cm}
...
\section{The Musil IDE}

The Musil IDE provides an integrated environment for writing, evaluating, and exploring Musil programs. It is designed to offer a lightweight but expressive workflow, combining a text editor, a console, and interactive tools for inspecting the runtime environment.

\subsection{Overview}

The IDE is organized into three main areas:

\begin{itemize}
    \item \textbf{Editor}: the main text area where Musil code is written and edited. It supports syntax highlighting, line numbers, and basic editing operations.
    \item \textbf{Console}: displays output produced by the program, including \texttt{print} statements and error messages.
    \item \textbf{Listener}: a small input field that allows evaluating single-line expressions interactively, similar to a REPL.
    \item \textbf{Environment Panel}: shows currently defined variables and procedures, allowing quick inspection of the program state.
\end{itemize}

\subsection{Basic Features}

The IDE includes the following core features:

\begin{itemize}
    \item \textbf{Run Script}: executes the entire content of the editor.
    \item \textbf{Run Selection}: executes only the selected portion of the code.
    \item \textbf{Stop}: interrupts a running computation (effective between evaluation steps).
    \item \textbf{Reset Environment}: clears all variables and procedures and restarts the Musil environment.
    \item \textbf{Syntax Highlighting}: highlights keywords, strings, comments, and structural elements.
    \item \textbf{Auto-completion}: triggered with \texttt{Ctrl+Tab}, suggests builtins and user-defined symbols.
    \item \textbf{Find/Replace}: supports searching and replacing text within the editor.
    \item \textbf{Zoom}: allows increasing or decreasing the font size.
    \item \textbf{Interactive Evaluation}: expressions can be evaluated directly in the listener without modifying the main script.
\end{itemize}

\subsection{File Management}

The IDE supports standard file operations:

\begin{itemize}
    \item Create new files
    \item Open existing files
    \item Save and Save As
\end{itemize}

The current file location is used as a reference for resolving relative paths during execution.

\subsection{Libraries and Installation}

To access the full capabilities of the language, including higher-level utilities and extended functions, it is necessary to install the Musil standard libraries.

This can be done directly from the IDE:

\begin{quote}
\texttt{Evaluate $\rightarrow$ Install libraries...}
\end{quote}

This operation copies the available \texttt{.mu} library files into the user directory (typically \texttt{\textasciitilde{}/.musil}), making them available to the \texttt{load} function.

Without installing the libraries, only the core built-in functionality of the language is available.

\subsection{Notes}

\begin{itemize}
    \item Long computations can be interrupted using the Stop command, but interruption occurs only at safe evaluation points.
    \item Output is streamed to the console during execution.
    \item The environment panel updates dynamically after each evaluation.
\end{itemize}
